...
mongoose
  .connect(process.env.DB_CONNECTION_TEST, { useNewUrlParser: true })
  .then(() => {
...
for (let i = 0; i < testData.length; i++) {
  testData[i].save(function (err, result) {
    done++;
    if (done === testData.length) {
      console.log("Seeding of test data complete.");
      exit();
    }
  });
}
